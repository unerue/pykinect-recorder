...
def test_skeleton2d_t():
    ...
    joints = [
        pykinect.k4abt_joint2D_t(skeleton2d_pos_array[i], skeleton2d_conf_array[i])
        for i in range(pykinect.K4ABT_JOINT_COUNT)
    ]
...
def test_empty_values():
    ...
    check_skeleton_t(
        skeleton_val,
        [[0, 0, 0] for i in range(pykinect.K4ABT_JOINT_COUNT)],
        [[0, 0, 0, 0] for i in range(pykinect.K4ABT_JOINT_COUNT)],
        [0 for i in range(pykinect.K4ABT_JOINT_COUNT)],
    )

    skeleton2d_val = pykinect.k4abt_skeleton2D_t()
    check_skeleton2d_t(
        skeleton2d_val,
        [[0, 0] for i in range(pykinect.K4ABT_JOINT_COUNT)],
        [0 for i in range(pykinect.K4ABT_JOINT_COUNT)],
    )
